...
% Reduce noise by multiplying the random values with a smaller factor
noise_factor = 0.2; % Adjust this factor to control the amount of noise reduction
x_actual_noise = x_ref + noise_factor * randn(size(x_ref));
x_actual_no_noise = x_ref;

% Plot the slot and trajectories with reduced noise
figure;
plot(t, t_edge, 'r', t, b_edge, 'r', t, x_ref, 'b', t, x_actual_noise, 'g');
legend('Top edge', 'Bottom edge', 'Reference trajectory', 'Actual trajectory with reduced noise', 'Location','northwest');
xlabel('Time (s)');
ylabel('Position (cm)');
title('Slot Trajectory with Reduced Noise')
...
% Calculate the commanded force trajectory with reduced noise
...
% Plot the commanded force trajectory with reduced noise
figure;
plot(t, F_noise);
xlabel('Time (s)');
ylabel('Commanded force (N)');
title('Commanded Force trajectory based on Control Law with Reduced Noise')